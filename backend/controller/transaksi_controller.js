...
export const addPeminjaman = async (req, res) => {
  ...
  const [getUserId, getBarangId] = await Promise.all([
    ...
    prisma.inventori.findUnique({ where: { id_barang: Number(item_id) } }),
  ]);
  ...
  if (getUserId && getBarangId) {
    try {
      ...
      if (result) {
        const item = await prisma.inventori.findUnique({
...
export const usageReport = async (req, res) => {
  ...
  try {
    ...
    const items = await prisma.inventori.findMany({
      where: {
        AND: [
    ...
    const analysis = items.map((item) => {
      ...
      const groupKey = group_by === "location" ? item.location : item.category;
